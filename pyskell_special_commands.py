...
def return_type_pyskell(function=None):
    if function is None:
        print("ftype :: (a -> b) -> String")
        return
    ...
    arguments = [safe_eval(arg) if is_valid_list_or_tuple(
    ...
    if function is not None and function.calleable():
        ...
        if callable(resultado):
            ...
            resultado_final = pyskellRunProccess(
                apply_args, resultado, arguments[1:])
            print(resultado_final.__str__() if isinstance(resultado_final, PyskellFunction)
                    else f"({command_string}) :: {type(resultado_final).__name__}")
        else:
            print(f"({command_string}) ::", resultado.__str__() if isinstance(
                resultado, PyskellFunction) else type(resultado).__name__)
    else:
